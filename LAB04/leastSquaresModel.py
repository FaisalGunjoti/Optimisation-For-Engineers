...
class leastSquaresModel:
    ...
    def residual(self, p: np.array):
        ...
        for i in range(self.N):
            value_r = self.model.objective(self.xData[:, i].reshape(self.n, 1))
            myResidual[i,:] = value_r - self.fData[:,i]
    ...
    def jacobian(self, p: np.array):
        ...
        # INCOMPLETE CODE STARTS
        for i in range(self.N):
            grad_j = self.model.parameterGradient(self.xData[:, i].reshape(self.n, 1))
            myJacobian[i,:] = grad_j.T
